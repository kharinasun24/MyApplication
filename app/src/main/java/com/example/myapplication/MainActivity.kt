...
groceryViewModel.items.value?.let { items ->
    if (items.isNotEmpty() && phoneNumber != null) {
     ...
     ///////////////////////////////////////
        ...
        // 2. Intent erstellen, um eine SMS zu senden.
        val smsIntent = Intent(Intent.ACTION_SENDTO).apply {
            setData(Uri.parse("smsto:$phoneNumber"))
            putExtra("sms_body", itemsString)
        }

        // 3. Überprüfen, ob eine Aktivität den Intent verarbeiten kann.
        if (smsIntent.resolveActivity(packageManager) != null) {
            startActivity(smsIntent)  // Startet die Standard-SMS-App.
        } else {
